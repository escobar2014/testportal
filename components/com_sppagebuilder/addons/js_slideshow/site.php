							...
							if(isset($item_value->slideshow_inner_items) && is_array($item_value->slideshow_inner_items)){
								foreach($item_value->slideshow_inner_items as $inner_item_key => $inner_value){
									...
									if($content_type == 'text_content'){
										$output .='<div id="'.$inner_uniqid.'" class="sppb-sp-slider-text" data-layer="true" data-animation=\'{'.$animation_settings.'}\'>';
										$output .= $content_text;
										$output .='</div>';
					...
					if($slider_animation != '3D'){
						if((isset($item_value->slider_video) && $item_value->slider_video) && !$item_value->enable_youtube_vimeo){
							if(strpos($item_value->slider_video, "http://") !== false || strpos($item_value->slider_video, "https://") !== false){
								$output .= '<div class="sp-video-background" data-video_src="'.$item_value->slider_video.'"></div>';
							} else {
								$output .= '<div class="sp-video-background" data-video_src="'. JURI::base() . $item_value->slider_video .'"></div>';
							}
		...
		<# 
			...
			#>
					...
					#>
								...
								#>
										<div id="{{inner_uniqid}}" class="sppb-sp-slider-text sp-editable-content" data-id={{data.id}} data-fieldName="content_text" data-layer="true" data-animation={{animationJson}}>
										...
										<div class="sppb-col-xs-{{12-image_column_width_xs}} sppb-col-sm-{{12-image_column_width_sm}} sppb-col-md-{{12-image_column_width}}">
											<div class="sp-slider-content-align-{{content_alignment}}">
												...
												_.each(item_value.slideshow_inner_items, function(inner_value, inner_item_key){
													...
													if(inner_value.content_type == "text_content"){
												#>
														<div id="{{inner_uniqid}}" class="sppb-sp-slider-text sp-editable-content" data-id={{data.id}} data-fieldName="content_text" data-layer="true" data-layer="true" data-animation={{animationJson}}>
															{{{inner_value.content_text}}}
														</div>
													<# } else if(inner_value.content_type == "btn_content"){ #>
														<a id="{{inner_uniqid}}" <# if(inner_value.button_target == "_blank"){ #> target="_blank" <# } #> class="sppb-sp-slider-button" href="{{inner_value.button_url}}" data-layer="true" data-animation={{animationJson}}>
						...
						#>
									<div class="sp-video-background" data-video_src="{{item_value.slider_video}}"></div>
								<# } else { #>
									<div class="sp-video-background" data-video_src="{{pagebuilder_base +  item_value.slider_video }}"></div>
								<# }
